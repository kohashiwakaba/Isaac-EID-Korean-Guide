...
!!! pied-piper "마지막 업데이트 : 2022-12-11"
    - 조합 가방 레시피가 정상적으로 작동되지 않던 문제 수정
    - CANTRIPPED 챌린지의 카드 설명이 보이도록 변경
    - D4류 아이템으로 획득한 아이템에 대한 변신세트 카운터 체크 개선
    - 아이템 해금 여부 체크 개선

!!! info "이전 업데이트 : 2022-12-09"
    - 구버전 표시 기준을 1.7.9b 미만으로 변경
    - 조합 가방 목록 표시 중 "Itempool Percentages" 명칭을 "Item Probability" 으로 변경(레시피 옵션 재설정 필요)

!!! info "이전 업데이트 : 2022-11-17"
    - 1.7.9a 버전 대응
    - False PHD가 탄속 감소 효과를 잘못 보여주던 문제 수정
    - Sanguine Bond 아이템 설명 표시 방식 개선

!!! info "이전 업데이트 : 2022-11-07"
    - --luadebug 및 SaveGame 기능 관련 오류 수정
    - 변신세트로 인한 설명 깜빡임 문제 수정
    - 알약 사용이 변신세트에 카운트 되지 않던 문제 수정

!!! info "이전 업데이트 : 2022-11-06"
    - 1.7.9 버전 및 조합 가방 레시피 대응
    - 모드 설명 지원 개선
    - Teleport 2.0 아이템이 비스트 루트 Mausoleum/Gehenna 및 거울세계에서 정상적으로 표시되지 않던 문제 수정
    - D4류 아이템으로 획득한 아이템에 대한 변신세트 카운터 체크 개선
    - 구버전 표시 기준을 1.7.9 미만으로 변경
