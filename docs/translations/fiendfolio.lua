...
EID:addTrinket(FiendFolio.ITEM.ROCK.PLACEBEODE, "무효과#흡수 시 추가 정동석 소지 효과를 받습니다.#!!! 석기 장신구 : {{ColorTeal}}정동석{{CR}} 타입", "위약 정동", "ko_kr")
EID:addTrinket(FiendFolio.ITEM.ROCK.WARM_GEODE, "캐릭터가 불 주변에 있는 동안 {{DamageSmall}}공격력/{{ShotspeedSmall}}탄속 증가#다른 정동석 소지 시 캐릭터가 불 주변에 있는 동안 {{Burning}}적에게 화상을 입히는 눈물을 발사합니다.#따뜻한 스테이지에서는 위 효과가 항상 발동합니다.#!!! 석기 장신구 : {{ColorTeal}}정동석{{CR}} 타입", "따뜻한 정동", "ko_kr")
EID:addTrinket(FiendFolio.ITEM.ROCK.CHAIN_GEODE, "장애물 파괴 시 40%의 확률로 인접한 장애물을 파괴합니다.#다른 정동석 소지 시 확률이 70%로 증가하며 추가로 20%의 확률로 인접한 구덩이를 메웁니다.#!!! 석기 장신구 : {{ColorTeal}}정동석{{CR}} 타입", "연쇄 정동", "ko_kr")
EID:addTrinket(FiendFolio.ITEM.ROCK.RUSTY_GEODE, "↑ {{DamageSmall}}서서히 감소하는 공격력 +8#다른 정동석 소지 시 공격력 감소 속도가 느려집니다.#!!! 석기 장신구 : {{ColorTeal}}정동석{{CR}} 타입", "녹슨 정동", "ko_kr")
...
EID:addTrinket(FiendFolio.ITEM.ROCK.VINYL_GEODE_A, "↑ {{DamageSmall}}공격력 +1#↑ {{ShotspeedSmall}}탄속 +0.24#짝수 스테이지 진입 시 뒷면으로 뒤집습니다.({{SpeedSmall}}/{{TearsSmall}} 증가)#다른 정동석 소지 시:#↑ {{SpeedSmall}}이동속도 +0.07#↑ {{TearsSmall}}눈물 딜레이 -0.6#!!! 석기 장신구 : {{ColorTeal}}정동석{{CR}} 타입", "비닐 정동 - 앞면", "ko_kr")
EID:addTrinket(FiendFolio.ITEM.ROCK.VINYL_GEODE_B, "↑ {{SpeedSmall}}이동속도 +0.2#↑ {{TearsSmall}}눈물 딜레이 -1.8#홀수 스테이지 진입 시 앞면으로 뒤집습니다.({{DamageSmall}}/{{ShotspeedSmall}} 증가)#다른 정동석 소지 시:#↑ {{DamageSmall}}공격력 +0.3#↑ {{ShotspeedSmall}}탄속 +0.08#!!! 석기 장신구 : {{ColorTeal}}정동석{{CR}} 타입", "비닐 정동 - 뒷면", "ko_kr")
